...
module.exports.run = async (bot, message, args, con) => {
    ...
    if (target.id === bot.user.id) return errors.infoBot(message);
    ...
    if(!success[target.id]){
        success[target.id] = {
          success: "Ce membre n'a pas encore obtenu aucun trophé.."
        };
    }
    ...
    if (target.lastMessage == null) {
        lastmsg = "Aucun message sur le serveur";
    }else {
        lastmsg = target.lastMessage;
    }
    ...
    .setAuthor("ℹ️ Infos utilisateur:")
    .setThumbnail(target.displayAvatarURL)
    .setColor("#15f153")
    .addField("❯ Pseudo", `${target}`, true)
    .addField("❯ Hashtag", `#${target.discriminator}`, true)
