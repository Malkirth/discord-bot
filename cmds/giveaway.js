...
module.exports.run = async (bot, message, args) => {
    if (args[0] == "help") {
        message.reply("Usage: !giveaway <nombre de gagnant> <durée> <récompense>");
        return;
    }
    ...
    winnerCount = args[0];
    if(!winnerCount) return message.reply("Vous devez spécifier un nombre de gagnant.")
    time = args[1];
    if(!time) return message.reply("Vous devez spécifier un temps de participation.")
    price = args.splice(2, args.length).join(' ');
    if(!price) return message.reply("Vous devez spécifier une récompense à ce giveaway.")
